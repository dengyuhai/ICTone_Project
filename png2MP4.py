...
def rgba_to_rgb(frame: np.ndarray, background: tuple[int, int, int] = (255, 255, 255)) -> np.ndarray:
    """
    MP4 不支持透明通道，因此把 RGBA 帧合成到纯色背景上。
    """
    if frame.shape[-1] == 3:
        return frame
...
def save_mp4(frames, out_path: str, fps: int = 18):
    rgb_frames = [rgba_to_rgb(frame) for frame in frames]
    imageio.mimsave(
        out_path,
        rgb_frames,
        fps=fps,
...
def main():
    ...
    save_mp4(all_frames, out_path="two_stage_result_keep_overlay.mp4", fps=18)
